...
[
  ...
  if test "x$ZLIB_VARIANT_CLOUDFLARE" == xtrue; then
    if { test "x$OPENJDK_TARGET_CPU" = "xx86_64" &&
      ...
      AC_MSG_ERROR([zlib variant 'cloudflare' currently only supported on Linux/Windows/MacOSX/x86_64 and Linux/aarch64])
    fi
  fi
  if test "x$ZLIB_VARIANT_CHROMIUM" == xtrue; then
    if { test "x$OPENJDK_TARGET_CPU" = "xx86_64" &&
      ...
      AC_MSG_ERROR([zlib variant 'chromium' currently only supported on Linux/Windows/MacOSX/x86_64 and Linux/aarch64])
    fi
  fi
